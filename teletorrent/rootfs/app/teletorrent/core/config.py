...
def build_telegram(cfg):
    ...
    if not users:
        log.warning("No authorized users configured")
...
def build_proxy(cfg):
    ...
    if not ptype or ptype not in ("socks", "http"):
        log.warning("proxy enabled but invalid type → disabling")
        return {"enabled": False, "url": None}

    # проверка host/port
    if not host or not port:
        log.warning("proxy enabled but host/port missing → disabling")
        return {"enabled": False, "url": None}
...
def main():
    ...
    log.info("Config init completed successfully")
