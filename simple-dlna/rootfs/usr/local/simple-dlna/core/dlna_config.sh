...
extract_current_managed_block() {
    ...
    awk '
        /<<< SIMPLE-DLNA-MANAGED-START >>>/ {capture=1}
        capture {print}
        /<<< SIMPLE-DLNA-MANAGED-END >>>/ {capture=0}
    ' "${CONFIG_FILE}"
}
...
sync_managed_block() {
    ...
    if ! grep -q "<<< SIMPLE-DLNA-MANAGED-START >>>" "${CONFIG_FILE}"; then
    ...
    awk '
        /<<< SIMPLE-DLNA-MANAGED-START >>>/ {skip=1; next}
        /<<< SIMPLE-DLNA-MANAGED-END >>>/ {skip=0; next}
        !skip
